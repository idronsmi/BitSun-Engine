...
use renderer::BsQueue;

use vulkano::buffer::{
    immutable::ImmutableBuffer, BufferAccess, BufferUsage, CpuBufferPool, TypedBufferAccess,
};
...
use vulkano::instance::{
    layers_list, Features, Instance, InstanceExtensions, PhysicalDevice, PhysicalDeviceType,
};
...
impl Application {
    pub fn new() -> Self {
        ...
        let (surface, events_loop) = Self::create_surface(&instance);
        ...
        let uniform_buffer_pool =
            CpuBufferPool::<vertex_shader::ty::UniformBufferObject>::new(device.clone(), BufferUsage::all());
    ...
    fn create_surface(instance: &Arc<Instance>) -> (Arc<Surface<Window>>, EventsLoop) {
